...
if ($current_user_role === 'Pimpinan') {
    ...
    // Karyawan tapi belum ada assignment ID
    $ids_to_query = [];
}
...
if ($jumlah_kandang_tampil > 0) {
    ...
    $result_populasi_agg = $koneksi->query("
     ...
     ");
    $agregat_populasi = [];
    if ($result_populasi_agg) {
        while ($row = $result_populasi_agg->fetch_assoc()) {
            $agregat_populasi[$row['id_kandang']] = $row;
        }
    }
    ...
    if ($result_beli) {
        while ($row = $result_beli->fetch_assoc()) {
            $beli_pakan_per_kandang[$row['id_kandang']] = $row['total_beli'];
        }
    }
    ...
    if ($result_pakai) {
        while ($row = $result_pakai->fetch_assoc()) {
            $pakai_pakan_per_kandang[$row['id_kandang']] = $row['total_pakai'];
        }
    }
    ...
    foreach ($kandang_aktif_list as $id_k => $kandang_data) {
        ...
        $data_per_kandang[$id_k] = [
            ...
            'populasi' => $populasi_saat_ini,
            'umur' => $umur_minggu,
            'sisa_pakan' => $sisa_pakan_kandang,
            'produksi_kemarin' => 0,
            'kematian_kemarin' => 0,
            'pakan_pakai_kemarin' => 0,
            'fcr_kemarin' => 'N/A',
            'prod_bulan_ini' => 0,
            'mati_bulan_ini' => 0,
            'prod_bulan_lalu' => 0,
        ...
        while ($row = $result_harian->fetch_assoc()) {
            ...
            if (isset($data_per_kandang[$id_k])) {
                if ($row['tanggal'] == $today) {
                    ...
                    $total_pakan_pakai_kemarin += $row['total_pakan'];
                    $total_telur_prod_kemarin += $row['total_produksi'];

                    // Hitung FCR Per Kandang
                    if ($row['total_produksi'] > 0) {
                        $data_per_kandang[$id_k]['fcr_kemarin'] = number_format($row['total_pakan'] / $row['total_produksi'], 2);
                    } elseif ($row['total_pakan'] > 0) {
                        $data_per_kandang[$id_k]['fcr_kemarin'] = '∞';
                    } else {
                        $data_per_kandang[$id_k]['fcr_kemarin'] = '0.00';
                    }
                }
            }
        }
    ...
    if ($total_telur_prod_kemarin > 0) {
        $total_fcr_kemarin = number_format($total_pakan_pakai_kemarin / $total_telur_prod_kemarin, 2);
    } elseif ($total_pakan_pakai_kemarin > 0) {
        $total_fcr_kemarin = '∞';
    }
    ...
    if ($result_grafik) {
        while ($row = $result_grafik->fetch_assoc()) {
            $produksi_bulanan[$row['tanggal']] = $row['total_produksi_harian'];
        }
    }
    ...
    try {
        ...
        error_log("Error creating date period for chart: " . $e->getMessage());
    }
} // --- Akhir Query Block ---
...
<div class="container-fluid">
                ...
                <div class="col-lg-6">
                    <div class="card shadow mb-4">
                        <div class="card-header bg-light py-3">
                            <h6 class="m-0 font-weight-bold text-secondary"><?php echo htmlspecialchars($data['nama_kandang']); ?> (<?php echo is_numeric($data['umur']) ? number_format($data['umur'], 1) . ' Mg' : 'N/A'; ?>)</h6>
                        </div>
                        <div class="card-body">
                            <div class="row mb-2">
                                <div class="col-6">
    ...
    <?php if ($current_user_role === 'Pimpinan' && $jumlah_kandang_tampil > 1): ?>
        <h4 class="mb-3"><i class="fas fa-table"></i> Ringkasan Performa Kandang (<?php echo $yesterday_formatted; ?>)</h4>
        <div class="row">
            <div class="col-lg-12">
                <div class="card shadow mb-4">
                    <div class="card-body">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Hapus pengecekan totalProduksiBulanIni dari sini
        // const totalProduksiBulanIni = chartDataProduksi.reduce((a, b) => a + b, 0);

        // --- PERBAIKAN 2: Hapus pengecekan totalProduksiBulanIni > 0 ---
        ...
        // Hapus inisialisasi DataTables untuk tabel ringkasan
        /*
