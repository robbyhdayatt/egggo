...
$yesterday = date('Y-m-d', strtotime('-1 day')); // Untuk FCR
...
$ids_string = !empty($ids_to_query) ? implode(',', $ids_to_query) : '0'; // String ID untuk klausa IN
...
$bulan_lalu_pakan = 0;
$total_produksi_today = 0; // Tambahan inisialisasi
$total_kematian_today = 0; // Tambahan inisialisasi

// *** PERBAIKAN: Tambahkan inisialisasi untuk variabel kemarin ***
...
if ($jumlah_kandang_tampil > 0) {
    ...
    // --- A. Agregat Populasi & Sisa Pakan (Saat Ini) ---
     ...
     foreach ($kandang_aktif_list as $id_k => $kandang_data) {
         ...
         $umur_minggu = ($kandang_data['umur_ayam_awal'] + $selisih_hari) / 7;
         ...
         $data_per_kandang[$id_k] = [
            ...
            'prod_bulan_ini' => 0, 'mati_bulan_ini' => 0, // Data baru
            'prod_bulan_lalu' => 0, 'mati_bulan_lalu' => 0 // Data baru
         ];
     }
            ...
            if(isset($data_per_kandang[$id_k])){ 
                if($row['tanggal'] == $today){
                    ...
                    $total_telur_prod_kemarin += $row['total_produksi']; // Akumulasi global
                     ...
                     elseif ($row['total_pakan'] > 0) { $data_per_kandang[$id_k]['fcr_kemarin'] = '∞'; }
                     else { $data_per_kandang[$id_k]['fcr_kemarin'] = '0.00'; }
                }
            }
        }
    }
    // Hitung FCR Total Kemarin (Sekarang variabel sudah diinisialisasi)
    ...
    if($result_bulanan){
        while($row = $result_bulanan->fetch_assoc()){
            $id_k = $row['id_kandang'];
            if(isset($data_per_kandang[$id_k])){ // Pastikan data kandang ada
...
<div class="container-fluid">
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-lg-6">
            <div class="card shadow h-100">
                <div class="card-header bg-primary text-white">
                    <h6 class="m-0 font-weight-bold"><i class="fas fa-calendar-check"></i> Performa Bulan Ini (<?php echo $nama_bulan_ini; ?>)</h6>
                </div>
                <div class="card-body p-0">
                    <ul class="list-group list-group-flush">
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            <span><i class="fas fa-egg me-2 text-info"></i> Total Produksi Telur</span>
                            <span class="badge bg-info rounded-pill fs-6"><?php echo number_format($bulan_ini_produksi, 2, ',', '.'); ?> Kg</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            <span><i class="fas fa-seedling me-2 text-success"></i> Total Penggunaan Pakan</span>
                            <span class="badge bg-success rounded-pill fs-6"><?php echo number_format($bulan_ini_pakan, 2, ',', '.'); ?> Kg</span>
                        </li>
                         <li class="list-group-item d-flex justify-content-between align-items-center">
                            <span><i class="fas fa-skull-crossbones me-2 text-danger"></i> Total Kematian + Afkir</span>
                            <span class="badge bg-danger rounded-pill fs-6"><?php echo number_format($bulan_ini_mati_afkir); ?> Ekor</span>
                        </li>
                    </ul>
                </div>
            </div>
        ...
        <div class="col-lg-6">
            <div class="card shadow h-100">
                <div class="card-header bg-secondary text-white">
                    <h6 class="m-0 font-weight-bold"><i class="fas fa-calendar-alt"></i> Performa Bulan Kemarin (<?php echo $nama_bulan_lalu; ?>)</h6>
                </div>
                 <div class="card-body p-0">
                     <ul class="list-group list-group-flush">
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            <span><i class="fas fa-egg me-2 text-muted"></i> Total Produksi Telur</span>
                            <span class="badge bg-light text-dark rounded-pill fs-6"><?php echo number_format($bulan_lalu_produksi, 2, ',', '.'); ?> Kg</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            <span><i class="fas fa-seedling me-2 text-muted"></i> Total Penggunaan Pakan</span>
                            <span class="badge bg-light text-dark rounded-pill fs-6"><?php echo number_format($bulan_lalu_pakan, 2, ',', '.'); ?> Kg</span>
                        </li>
                         <li class="list-group-item d-flex justify-content-between align-items-center">
                            <span><i class="fas fa-skull-crossbones me-2 text-muted"></i> Total Kematian + Afkir</span>
                            <span class="badge bg-light text-dark rounded-pill fs-6"><?php echo number_format($bulan_lalu_mati_afkir); ?> Ekor</span>
                        </li>
                    </ul>
                </div>
            </div>
        ...
        <div class="col-lg-12">
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <h6 class="m-0 font-weight-bold text-primary"><i class="fas fa-chart-area me-1"></i> Grafik Produksi Telur Harian (<?php echo $nama_bulan_ini; ?>)</h6>
                </div>
                <div class="card-body">
                    ...
                    <?php else: ?>
                        <div class="alert alert-warning text-center">Data produksi bulanan belum tersedia untuk ditampilkan.</div>
                    <?php endif; ?>
                </div>
            </div>
        </div>
    </div><?php if ($current_user_role === 'Pimpinan' && !empty($data_per_kandang)): ?>
         <hr>
         <h4 class="mb-3"><i class="fas fa-home"></i> Detail Per Kandang (Aktif)</h4>
         <div class="row g-3">
             <?php foreach ($data_per_kandang as $id_k => $data): ?>
                <?php
                // Ambil data bulanan untuk kandang ini dari array yang sudah di-fetch
                ...
                <div class="col-lg-6">
                    <div class="card shadow mb-4">
                        <div class="card-header bg-light py-3">
                            <h6 class="m-0 font-weight-bold text-secondary"><?php echo htmlspecialchars($data['nama_kandang']); ?> (<?php echo number_format($data['umur'], 1); ?> Mg)</h6>
                        </div>
                        <div class="card-body">
                            <div class="row mb-2">
                                <div class="col-6">
                                    <div class="text-xs font-weight-bold text-primary text-uppercase mb-1">Populasi</div>
                                    <div class="h5 mb-0 font-weight-bold text-gray-800"><?php echo number_format($data['populasi']); ?></div>
                                </div>
                                <div class="col-6">
                                    <div class="text-xs font-weight-bold text-secondary text-uppercase mb-1">Sisa Pakan</div>
                                    <div class="h5 mb-0 font-weight-bold text-gray-800"><?php echo number_format($data['sisa_pakan'], 2, ',', '.'); ?> Kg</div>
                                </div>
                            </div>
                            <hr class="my-2">
                            ...
                            <ul class="list-group list-group-flush mb-2" style="font-size: 0.85rem;">
                                <li class="list-group-item d-flex justify-content-between align-items-center p-1 px-2">
                                    ...
                                    <span class="font-weight-bold"><?php echo number_format($k_prod_bi, 2, ',', '.'); ?> Kg</span>
                                </li>
                                <li class="list-group-item d-flex justify-content-between align-items-center p-1 px-2">
                                    <span>Mati/Afkir:</span>
                                    <span class="font-weight-bold"><?php echo number_format($k_mati_bi); ?> Ekor</span>
                                </li>
                            </ul>
                             ...
                             <ul class="list-group list-group-flush" style="font-size: 0.85rem;">
                                <li class="list-group-item d-flex justify-content-between align-items-center p-1 px-2">
                                    <span>Produksi Telur:</span>
                                    <span class="font-weight-bold"><?php echo number_format($k_prod_bl, 2, ',', '.'); ?> Kg</span>
                                </li>
                                <li class="list-group-item d-flex justify-content-between align-items-center p-1 px-2">
                                    <span>Mati/Afkir:</span>
                                    <span class="font-weight-bold"><?php echo number_format($k_mati_bl); ?> Ekor</span>
                                </li>
                            </ul>
                        </div>
                    </div>
     ...
     <?php if ($current_user_role === 'Pimpinan' && $jumlah_kandang_tampil > 1): ?>
         <h4 class="mb-3"><i class="fas fa-table"></i> Ringkasan Performa Kandang (Data Kemarin)</h4>
         <div class="row">
             <div class="col-lg-12">
                 <div class="card shadow mb-4">
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped table-hover" id="tabelRingkasanKandang" width="100%" cellspacing="0" style="font-size: 0.9rem;">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-end"><?php echo number_format($data['umur'], 1, ',', '.'); ?></td>
                                        ...
                                        <td class="text-end"><?php echo $data['fcr_kemarin']; ?></td>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const isPimpinan = <?php echo ($current_user_role === 'Pimpinan') ? 'true' : 'false'; ?>;
    ...
    // Inisialisasi DataTables untuk tabel ringkasan
     if ($.fn.DataTable && $('#tabelRingkasanKandang').length) { 
        $('#tabelRingkasanKandang').DataTable({
            "language": { "url": "https://cdn.datatables.net/plug-ins/1.13.7/i18n/id.json" },
            "order": [[ 0, "asc" ]], 
            // Aktifkan fitur interaktif HANYA untuk Pimpinan
            "searching": isPimpinan, 
            "paging": isPimpinan, 
            "info": isPimpinan, 
            "lengthChange": isPimpinan 
        });
     }
});
</script>
